...
-(id)initWithApiKey:(NSString *)key customHeaders:(NSDictionary *)customHeaders
{
  ...
  for (NSString *key in customHeaders)
    [headers setValue:customHeaders[key] forKey:key];
  
  return self;
}


-(void)configure:(NSString *)key
{
  apiUrl  = API_URL;
  apiKey  = key;
...
-(void)figureOutVersions
{
  ...
  NSString *versionHeader = [NSString stringWithFormat:@"what3words-ObjC/%@ (ObjC %d; %@ %ld.%ld.%ld)", api_version, objc_version, os_name, (long)os_version.majorVersion, (long)os_version.minorVersion, (long)os_version.patchVersion];
  NSString *bundleHeader  = [[NSBundle mainBundle] bundleIdentifier];
  
  [headers setValue:versionHeader forKey: @"X-W3W-Wrapper"];
  [headers setValue:bundleHeader forKey: @"X-Ios-Bundle-Identifier"];
}
